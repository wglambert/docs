...
```console
$ docker run --name tmp-nginx-container -d mips64le/nginx
...
To run mips64le/nginx in read-only mode, you will need to mount a Docker volume to every location where mips64le/nginx writes information. The default mips64le/nginx configuration requires write access to `/var/cache` and `/var/run`. This can be easily accomplished by running mips64le/nginx as follows:
